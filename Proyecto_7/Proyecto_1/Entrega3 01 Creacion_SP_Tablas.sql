...
---VALIDAR EXISTENCIA DEL PRODUCTO
	IF(@MSJ = '' AND @id_producto NOT IN (SELECT id_Producto FROM grupo3.Producto))
	SET @MSJ = @MSJ + 'EL PRODUCTO INGRESADO ES INEXISTENTE'
	ELSE
BEGIN
	SET @Precio_Unitario = (SELECT Precio_Unitario FROM grupo3.Producto WHERE id_producto = @id_producto)
END
...
BEGIN
	...
	SET Cantidad = @Cantidad,
		...
		Precio_Unitario = (SELECT Precio_Unitario FROM grupo3.Producto WHERE id_producto = @id_producto),
		id_Venta = @id_Venta
...
GO 
CREATE OR ALTER PROCEDURE insert3.InsertarFacturayVenta
	@Numero_Factura Char(11),
	@tipo_factura char(2),
	@metodo_pago varchar (15),
	@Identificador_de_pago varchar(30),
...
EXEC insert3.Insertar_Detalle @Cantidad, @id_producto, @id_Venta
...
GO
CREATE OR ALTER PROCEDURE delete3.EliminarNotaDeCredito (@id_NotaDeCredito int)
AS
BEGIN
...
GO
CREATE OR ALTER PROCEDURE update3.ModificarNotaDeCredito (
