...
begin
	...
	LEFT JOIN administracion3.Producto p ON pt.Nombre = p.Nombre
...
begin 
    ...
    SELECT Ciudad, [Reemplazar por] as Sucursal, direccion, Horario, Telefono
     FROM OPENROWSET(
		...
		WHERE [Reemplazar por] NOT IN (SELECT Sucursal FROM administracion3.Sucursal)';
...
begin 
    ...
    ) WHERE [Legajo/ID] IS NOT NULL AND DNI NOT IN (SELECT DNI FROM administracion3.Empleado)';

EXEC sp_executesql @sql;


	INSERT INTO administracion3.Empleado(Nombre,Apellido,DNI,Direccion,email_Personal,email_Empresa,Cargo,id_Sucursal,Turno)
	SELECT e.Nombre, e.Apellido, e.DNI, e.Direccion ,REPLACE(REPLACE(e.email_Personal,' ',''),CHAR(9),''),
	       REPLACE(REPLACE(e.email_Empresa,' ',''),CHAR(9),''),e.Cargo, s.id_sucursal, e.Turno
	FROM administracion3.Sucursal s
	INNER JOIN grupo3.#Empleado_Temp e ON s.Sucursal = e.Sucursal

	UPDATE administracion3.Empleado
	SET CUIL = '00-'+CAST(DNI AS VARCHAR)+'-0'
	WHERE CUIL NOT in (SELECT CUIL FROM administracion3.Empleado)
...
BEGIN
	...
	SET @Estado = 'Pagada'
	DECLARE @cant_Fac int
	SET @cant_Fac = (SELECT count(id_Factura) FROM ventas3.Factura)
	...
	CREATE TABLE grupo3.#cte_Temp(Genero varchar(10),
								 cuil_cte char(13))	

	INSERT INTO grupo3.#cte_Temp(Genero,Cuil_cte)
	VALUES('Male','223-39-5632'),('Female','567-22-1900')
	...
	INSERT INTO ventas3.Factura(Numero_Fac,Tipo_Factura,Fecha,Hora,Metodo_Pago,Identificador_de_pago)
	SELECT v.Numero_Fac, v.Tipo_Factura, v.Fecha, v.Hora, v.Metodo_Pago, v.identificador_de_pago
	FROM  grupo3.#Venta_Temp v
	where (v.Numero_Fac NOT IN (SELECT Numero_Fac FROM ventas3.Factura) AND v.Producto IN (SELECT Nombre FROM administracion3.Producto))

IF(@cant_Fac < (SELECT COUNT(id_Factura) FROM ventas3.Factura))
BEGIN
	UPDATE ventas3.Factura
	SET Estado = @Estado
	where Factura.Numero_Fac IN (SELECT Numero_Fac FROM grupo3.#Venta_Temp)

	INSERT INTO ventas3.Venta(Tipo_Cliente,Genero,id_Sucursal,id_Empleado,id_Factura,cuil_cte,Fecha)
	SELECT v.Tipo_Cliente, v.Genero, s.id_sucursal, e.id_Empleado, f.id_Factura,c.Cuil_cte, v.Fecha
	FROM grupo3.#Venta_Temp v
	INNER JOIN ventas3.Factura f ON v.Numero_Fac = f.Numero_Fac
	INNER JOIN administracion3.Sucursal s ON v.Ciudad = s.Ciudad
	INNER JOIN administracion3.Empleado e ON v.Empleado = e.id_Empleado
	INNER JOIN grupo3.#cte_Temp c ON v.Genero = c.Genero
	where f.id_Factura NOT IN (SELECT id_Factura FROM ventas3.Venta)

	INSERT INTO  ventas3.Detalle_Venta(Cantidad, id_Producto,Precio_Unitario, id_Venta)
	SELECT v.Cantidad, p.id_Producto, p.Precio_Unitario, vt.id_Venta
	FROM grupo3.#Venta_Temp v
	LEFT JOIN administracion3.Producto p ON v.Producto = p.Nombre
	INNER JOIN ventas3.Factura f ON v.Numero_Fac = f.Numero_Fac
	INNER JOIN ventas3.Venta vt ON vt.id_Factura = f.id_Factura
	WHERE (p.id_Producto NOT IN (SELECT id_Producto FROM ventas3.Detalle_Venta dv where dv.id_Venta = vt.id_Venta))

	UPDATE ventas3.Factura
SET Total = (
    SELECT SUM(dv.cantidad * dv.precio_unitario)
    FROM ventas3.Venta v
    JOIN ventas3.Detalle_Venta dv ON v.id_venta = dv.id_venta
    WHERE v.id_factura = Factura.id_factura),
	Total_IVA = (SELECT SUM(dv.cantidad * dv.precio_unitario) * 1.21
    FROM ventas3.Venta v
    JOIN ventas3.Detalle_Venta dv ON v.id_venta = dv.id_venta
    WHERE v.id_factura = Factura.id_factura)
END
	...
	DROP TABLE IF EXISTS grupo3.#cte_Temp
END
GO
...
BEGIN
    ...
    SELECT [Product] as Nombre, [Precio Unitario en dolares] as Precio_Unitario
     FROM OPENROWSET(
        ...
        ''Excel 12.0; Database=' + @ruta + ''',
        ''SELECT * FROM [Sheet1$]'')
		WHERE [Product] NOT IN (SELECT Nombre FROM administracion3.Producto)';

EXEC sp_executesql @sql;
	INSERT INTO administracion3.Producto(Categoria,Linea_De_Producto,Nombre,Precio_Unitario,Precio_De_Referencia,
...
CREATE TABLE grupo3.#Productos_Importados( id_producto int,
    ...
    SELECT [idProducto] as id_producto, [NombreProducto] as Nombre, [Categoría] as Categoria,
     ...
     FROM OPENROWSET(
		...
		WHERE [NombreProducto] NOT IN (SELECT Nombre FROM administracion3.Producto)';
