...
BEGIN
    ...
    FROM 
        ventas3.Detalle_Venta d
        INNER JOIN administracion3.Producto p ON p.id_Producto = d.id_Producto
        INNER JOIN ventas3.Venta v ON d.id_Venta = v.id_Venta
    WHERE
...
BEGIN
    ...
    FROM
        administracion3.Producto p
        INNER JOIN ventas3.Detalle_Venta d ON p.id_Producto = d.id_Producto
        INNER JOIN ventas3.Venta v ON d.id_Venta = v.id_Venta
        INNER JOIN administracion3.Empleado e ON v.id_Empleado = e.id_Empleado
        INNER JOIN administracion3.Sucursal s ON e.id_Sucursal = s.id_sucursal
    WHERE
        v.fecha BETWEEN @fechaI AND @fechaF
    GROUP BY
        s.Ciudad, 
...
BEGIN
    WITH VentasSemanales (id_Producto, Nombre, Semana, CantidadSemanal, Top5) AS (
        ...
        FROM
            ventas3.Detalle_Venta d
            INNER JOIN administracion3.Producto p ON d.id_Producto = p.id_Producto
            INNER JOIN ventas3.Venta v ON d.id_Venta = v.id_Venta
        WHERE 
            MONTH(v.Fecha) = @mes AND YEAR(v.Fecha) = @anio
        GROUP BY 
...
BEGIN
	...
	FROM ventas3.Detalle_Venta d
        INNER JOIN ventas3.Venta v ON d.id_Venta = v.id_Venta
		INNER JOIN ventas3.Factura f ON f.id_Factura = v.id_Factura
        INNER JOIN administracion3.Producto p ON d.id_Producto = p.id_Producto
		INNER JOIN administracion3.Sucursal s ON v.id_Sucursal = s.id_sucursal
END
GO